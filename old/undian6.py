...
def start_doorprize():
    ...
    for _ in range(shuffle_steps):
        ...
        # konstant
        digit_35_1.set(random.randint(0, 9))
        digit_35_2.set(random.randint(0, 9))
        play_counting_sound()
        root.update()
    ...
    while final_number in winners:
        ...
        if not available_numbers:
            ...
            messagebox.showwarning("No more  numbers to draw.")
            start_button.config(state=tk.NORMAL)
            return
        final_number = random.choice(available_numbers)
    ...
    # konstant
    digit_35_1.set(int(3))
    digit_35_2.set(int(5))
