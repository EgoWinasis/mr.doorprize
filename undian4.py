...
# Fetch winners from database on startup
def fetch_winners_from_db():
    global winners
    if cursor:
        cursor.execute("SELECT winner_number FROM winners")
        result = cursor.fetchall()
        winners = {tuple(row[0]) for row in result}  # Convert to set of tuples
...
def save_winner_to_db(winner):
    if cursor and db:
        try:
            cursor.execute("INSERT INTO winners (winner_number) VALUES (%s)", ("".join(winner),))
            db.commit()
        except mysql.connector.Error as err:
...
# Initialize winners from database
...
def start_doorprize():
    ...
    # Save to database
    save_winner_to_db(final_number_str)  
